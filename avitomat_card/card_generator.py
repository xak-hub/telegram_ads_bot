...
def generate_card(config: CardConfig) -> str:
    ...
    # Всегда в ОДНУ строку: если не помещается — шрифт уменьшается (вплоть
    # до min_size в _wrap_title), без переноса на вторую строку.
    title_max_w = W - 2 * LOGO_MARGIN
    title_font, title_lines = _wrap_title(
        draw, config.title, FONT_BOLD_PATH, TITLE_FONT_SIZE, title_max_w,
        TITLE_LINE_GAP, max_lines=1)

...
def build_values(
    ...
    values = {
        ...
        # Видео (YouTube/Rutube) на все объявления — ссылкой из .env (VIDEO_URL).
        # Пусто = без видео. Пишем в обе видео-колонки шаблона Авито.
        "Ссылка на видео": os.environ.get("VIDEO_URL", "").strip(),
        "URL видеофайла": os.environ.get("VIDEO_URL", "").strip(),
    }
